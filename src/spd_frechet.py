...
def generate_scale_matrix(X: torch.Tensor, p: int = 5) -> torch.Tensor:
    """
    ...
    Args:
        ...
        p: dimension of the SPD matrix

    Returns:
        Sigma: [p, p] or [B, p, p] diagonal SPD scale matrix
    """
    squeeze = False
    if X.dim() == 1:
        X = X.unsqueeze(0)
    ...
    diag_entries = [s11, s22, s33, s44, s55]
    diag = torch.stack(diag_entries[:p], dim=-1).clamp(min=1e-8)
    Sigma = torch.diag_embed(diag)  # [B, p, p]
...
class WishartSPDDataset(Dataset):
    ...
    Data generation (paper pages 19-20):
      ...
      - Σ(X) = diag(Σ11, ..., Σpp) with input-dependent entries
      - Y ~ Wishart(Σ(X), df)  — V × p×p SPD matrix responses per sample
    ...
    Args:
        ...
        p: dimension of SPD matrices (default: 5)
    """

    def __init__(self, n: int, n_responses: int = 1, df: int = 6, seed: int = 42, p: int = 5):
        super().__init__()
        self.n = n
        self.n_responses = n_responses
        self.df = df
        self.p = p  # SPD matrix dimension
        ...
        self.Sigma = generate_scale_matrix(self.X, p=self.p)  # [n, p, p]
        ...
        for i in range(n):
            ...
            for v in range(n_responses):
                ...
                Yi_list.append(Yiv)
            Y_list.append(torch.stack(Yi_list))  # [V, p, p]
        self.Y = torch.stack(Y_list)  # [n, V, p, p]
        ...
        self.true_mean = self.df * self.Sigma.unsqueeze(1).expand(-1, n_responses, -1, -1)  # [n, V, p, p]
...
def evaluate_frechet_model(
    ...
    for X_batch, Y_batch in test_loader:
        ...
        if W.dim() == 2:
            ...
            d = dist_fn(m_w, Y_batch)  # [B]
            all_dists += d.cpu().tolist()
            if true_means is not None:
                tm = true_means[idx_offset: idx_offset + B].to(device)  # [B, p, p] or [B, 1, p, p]
                if tm.dim() == 4 and tm.shape[1] == 1:
                    tm = tm[:, 0, :, :]
                d_mean = dist_fn(m_w, tm)
                all_dists_to_mean += d_mean.cpu().tolist()
        else:
            # Multi-response
            V = W.shape[2]
            d_batch = []
            ...
            for v in range(V):
                ...
                d_batch.append(d_v)
                if true_means is not None:
                    tm_v = true_means[idx_offset: idx_offset + B, v, :, :]  # [B, p, p]
            ...
            all_dists += d_avg.cpu().tolist()
            if true_means is not None:
                d_mean_avg = torch.stack(d_mean_batch).mean(dim=0)  # [B]
                all_dists_to_mean += d_mean_avg.cpu().tolist()
