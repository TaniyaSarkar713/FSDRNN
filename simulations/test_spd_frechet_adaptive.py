...
def run_single_metric(
    ...
    if verbose:
        ...
        print(f"{'='*60}")
        print(f"  {'Method':<25} {'avg d(pred,Y)':>14} {'Improv%':>8} {'Time(s)':>8}")
        print(f"  {'-'*57}")

        def _imp(val):
            return (base_avg - val) / base_avg * 100 if base_avg > 0 else 0

        print(f"  {'Global Mean':<25} {base_avg:>14.6f} {'---':>8} {'0.0':>8}")
        print(f"  {'GFR':<25} {gfr_avg:>14.6f} {_imp(gfr_avg):>7.1f}% {gfr_time:>7.1f}")
        print(f"  {'DFR':<25} {dfr_avg:>14.6f} {_imp(dfr_avg):>7.1f}% {dfr_time:>7.1f}")
        print(f"  {'FSDRNN (d=':<8}{_fdrnn_rd}, adaptive){'':<10} {fdrnn_avg:>14.6f} {_imp(fdrnn_avg):>7.1f}% {fdrnn_time:>7.1f}")
...
def main():
    ...
    for metric_name in metrics_to_run:
        ...
        if args.tune:
            ...
            # --- FDRNN adaptive LoRA tuning: d then params ---
            adaptive_r = min(5, args.n_responses)
            print(f"  Chosen r = {adaptive_r}")
            ...
            print(f"  --- Chosen FDRNN (d={chosen_d}, adaptive r) params: {fdrnn_tuned_params}")
            print()
        ...
        print(f"  reduction dim d chosen from grid search (structural d0={structural_dim})")
        if dfr_tuned_params:
            print(f"  DFR tuned:          {dfr_tuned_params}")
        if fdrnn_tuned_params:
            print(f"  FSDRNN (d={chosen_d}, adaptive r) tuned:  {fdrnn_tuned_params}")
        print(f"{'#'*70}")
        ...
        if fdrnn_tuned_params:
            header_lines.append(f"  FSDRNN (adaptive) tuned:  {fdrnn_tuned_params}")
