...
class SharedFactorsSPDDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, n: int, case: str = "NL", seed: int = 42, p: int = 20, m: int = 4, V: int = 15, B0=None, U=None, S0=None, A=None, R=None):
        """
        ...
        if case in ["NLH", "NLR"]:
            ...
            b_v = torch.tensor([0.0, 0.05, 0.0, 0.05, 0.0, 0.05, 0.03, 0.0, 0.04, 0.02, 0.0, 0.06, 0.01, 0.03, 0.0])
...
def main():
    ...
    parser.add_argument("--output_file", type=str, default="spd_shared_factors_v15.csv")
    ...
    # Main experiment
    print(f"\n{'#'*70}")
    print(f"  SPD SHARED FACTORS (V=15): {args.case}  |  {n_reps} repetitions")
    print(f"{'#'*70}")
    ...
    if not args.no_tune:
        ...
        ds_tune = SharedFactorsSPDDataset(n=100, case=args.case, seed=args.seed, p=20, m=4, V=15)
