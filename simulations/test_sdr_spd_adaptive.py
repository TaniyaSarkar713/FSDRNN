...
def run_sdr_simulation(
    ...
    if verbose:
        ...
        print(f"  {'FDRNN (adaptive LoRA)':<20}{''}{'':<10} {fdrnn_avg:>14.6f} {_imp(fdrnn_avg):>7.1f}%")
        ...
        print(f"  {'-'*50}")
        for tag, tr, te in [
            ("FDRNN (adaptive LoRA)", fdrnn_train_avg, fdrnn_avg),
        ]:
...
def main():
    ...
    if fdrnn_tuned_params:
        print(f"  FDRNN (d={chosen_d}, adaptive r) tuned:  {fdrnn_tuned_params}")
    print(f"{'#'*70}")
    ...
    for rep in range(n_reps):
        ...
        if not verbose_rep:
            print(f"  FDRNN(adaptive LoRA)={res['fdrnn_avg_dist']:.4f}")
    ...
    if not np.isnan(resp_eff_mean):
        header_lines.append(f"  FDRNN adaptive LoRA rank metric: {resp_eff_mean:.2f} (SE {resp_eff_se:.2f})")
