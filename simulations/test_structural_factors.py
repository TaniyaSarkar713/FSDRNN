...
def run_single_case(
    ...
    if verbose:
        ...
        print(f"  {'IFR':<30} {ifr_avg:>14.6f} {_format_amspe(ifr_amspe)}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Structural Factors Simulation"
    )
    parser.add_argument(
        "--case",
        type=str,
        default="B",
        choices=["A", "B", "C"],
        help="Case: A (easier), B (main), C (stronger FDRNN advantage)"
    )
    ...
    parser.add_argument("--output_file", type=str, default="structural_factors_results.csv")
    parser.add_argument("--metric", type=str, default="frobenius", 
                        choices=["frobenius", "affine_invariant", "power", "log_cholesky", "bures_wasserstein"],
                        help="Distance metric for SPD manifold")

    args = parser.parse_args()

    # ==============================================================
    # FDRNN Hyperparameter Tuning (once per metric/case)
    # ==============================================================
    print(f"Performing FDRNN hyperparameter tuning for case {args.case}, metric {args.metric}...")
    
    # Create tuning dataset (subset of training data)
    tune_n = 200  # smaller dataset for tuning
    tune_seed = 0  # fixed seed for reproducibility
    
    # Generate shared structural parameters for tuning
    ...
    a = torch.zeros(15, 3)  # V=15
    for v in range(15):
        cluster = v // 5  # 0,1,2 for clusters 1,2,3
        noise = torch.randn(3) * 0.05
        a[v] = centers[cluster] + noise

    # Response-specific deviation matrices R_v [15, 4, 4]
    R = torch.zeros(15, 4, 4)
    for v in range(15):
        R[v] = torch.randn(4, 4) * 0.1
        R[v] = R[v] @ R[v].T  # make PSD

    c_v = torch.zeros(15)
    omega_v = torch.zeros(15, 4)
    d_v = torch.zeros(15)
    for v in range(15):
        c_v[v] = 0.5 + torch.randn(1) * 0.1
        omega_v[v] = torch.randn(4) * 0.1
        if v % 3 == 0:
            d_v[v] = -0.05
        elif v % 3 == 1:
            d_v[v] = 0.0
    ...
    tune_ds = StructuralFactorsDataset(tune_n, case=args.case, seed=tune_seed, shared_params=tune_shared_params)

    # Stage A: Architecture selection (d, reduction_type, lr)
    print(f"\n  === FDRNN Stage A: architecture selection ===")
    stage_a_configs = [
    ...
    for i, config in enumerate(stage_a_configs):
        ...
        print(f"\n    --- Config {i+1}/{len(stage_a_configs)}: d={d}, type={rt}, lr={lr:.1e} ---")
        ...
        result = grid_search_frechet(
            ...
            fixed_model_kwargs={
                "input_dim": 30,  # p=30
                "n_responses": 15,  # V=15
                "response_rank": 3,  # fixed for Stage A
                "encoder_sizes": [128, 64],
                "head_sizes": [64, 128],
            },
            fixed_train_kwargs={"epochs": 30},  # Reduced epochs for tuning
            val_frac=0.2,
    ...
    print(f"\n  === FDRNN Stage B: regularization tuning for d={chosen_d}, type={chosen_rt} ===")
    fdrnn_stage_b_grid = {
        "lr": [chosen_lr],  # fixed from Stage A
    ...
    fdrnn_result = grid_search_frechet(
        ...
        fixed_model_kwargs={
            "input_dim": 30,  # p=30
            "n_responses": 15,  # V=15
            "response_rank": 3,  # fixed response rank
            "encoder_sizes": [128, 64],
            "head_sizes": [64, 128],
        },
        fixed_train_kwargs={"epochs": 30},  # Reduced epochs for tuning
    ...
    fdrnn_tuned_params["response_rank"] = 3  # ensure it's set
    fdrnn_tuned_params["encoder_sizes"] = [128, 64]  # fixed architecture
    fdrnn_tuned_params["head_sizes"] = [64, 128]  # fixed architecture
    
    print(f"\n  Tuned FDRNN params: {fdrnn_tuned_params}")

    # ==============================================================
    # Main experiment repetitions
    # ==============================================================
    all_results = []
    ...
    # AMSPE bar chart
    print("\nGenerating AMSPE bar chart...")
    try:
        import matplotlib.pyplot as plt
        
        # Collect AMSPE values
        methods = []
        amspe_means = []
        amspe_ses = []
        ...
        for key, name in zip(amspe_keys, method_names):
            ...
            if values:
                ...
                se = np.std(values, ddof=1) / np.sqrt(len(values)) if len(values) > 1 else 0
                methods.append(name)
                amspe_means.append(mean)
                amspe_ses.append(se)
        
        if methods:
            # Create bar chart
            fig, ax = plt.subplots(figsize=(10, 6))
            bars = ax.bar(methods, amspe_means, yerr=amspe_ses, capsize=5, 
                         color=['lightcoral', 'lightblue', 'lightgreen', 'lightyellow', 'lightpink', 'lightcyan'][:len(methods)])
            
            ax.set_ylabel('AMSPE (Mean Squared Prediction Error)')
            ax.set_title(f'AMSPE Comparison - Case {args.case} ({args.metric})')
            ax.grid(axis='y', alpha=0.3)
            
            # Add value labels on bars
            for bar, mean, se in zip(bars, amspe_means, amspe_ses):
                height = bar.get_height()
                ax.text(bar.get_x() + bar.get_width()/2., height + se + 0.001,
                       '.6f', ha='center', va='bottom', fontsize=9)
            
            plt.tight_layout()
            plot_file = args.output_file.replace('.csv', '_amspe.png')
            plt.savefig(plot_file, dpi=150, bbox_inches='tight')
            print(f"AMSPE plot saved to {plot_file}")
            plt.close()
        else:
            print("No AMSPE data available for plotting")
            
    except ImportError:
        print("matplotlib not available, skipping AMSPE plot")
    except Exception as e:
        print(f"Error creating AMSPE plot: {e}")
...
def ifr_loss(theta, X, Y, dist_name, differentiable_frechet_mean, h):
    ...
    for i in range(n):
        ...
        true_y_tensor = torch.tensor(Y[i], dtype=torch.float32)
