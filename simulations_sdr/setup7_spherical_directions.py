...
def grid_search_fsdrnn_d(X_train, Y_train, p=20, V=15, d_values=[2, 3, 5],
                         ...
                         hidden_dim=32, dropout=0.1, device='cpu', verbose=False, val_split=0.2):
    """
...
def run_simulation(n_train=300, n_test=150, seed=42, device='cpu', verbose=False):
    ...
    with MethodTimer('Oracle FSDRNN') as timer:
        ...
        X_tr_oracle, Y_tr_oracle = X_train[tr_idx_oracle], Y_train[tr_idx_oracle]
        X_val_oracle, Y_val_oracle = X_train[val_idx_oracle], Y_train[val_idx_oracle]
        ...
        for r in range(oracle_restarts):
            ...
            val_error = evaluate_angular_distance(Y_val_oracle, val_pred)
    ...
    results['methods']['Oracle FSDRNN'] = {
        ...
        'best_val_angular_error': best_oracle_val
    }
    ...
    if verbose:
        ...
        print(f"  Oracle best val angular error: {best_oracle_val:.6f}")
