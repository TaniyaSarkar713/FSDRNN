...
def generate_synthetic_data(n, d0=2, p=20, V=8, noise_std=0.25, seed=42):
    """
...
def run_simulation(n_train=300, n_test=150, seed=42, verbose=False):
    ...
    start_time = time.time()
    method = DFRWrapper(20, 8, hidden_dim=32, lr=5e-4, epochs=1000, device=device, verbose=False)
    method.fit(X_train, Y_train)
    Y_pred = method.predict(X_test)
    elapsed = time.time() - start_time
    eval_metrics = evaluate_prediction(Y_test, Y_pred)
    ...
    start_time = time.time()
    method = E2MWrapper(20, 8, latent_dim=3, lr=5e-4, epochs=1000, device=device, verbose=False)
    method.fit(X_train, Y_train)
    Y_pred = method.predict(X_test)
    elapsed = time.time() - start_time
    eval_metrics = evaluate_prediction(Y_test, Y_pred)
    ...
    start_time = time.time()
    method = FSdrnnWrapper(20, 8, d=2, lr=5e-4, epochs=1000, dropout=0.1, device=device, verbose=False)
    method.fit(X_train, Y_train)
    Y_pred = method.predict(X_test)
    elapsed = time.time() - start_time
    eval_metrics = evaluate_prediction(Y_test, Y_pred)
    ...
    # ========================================================================
    # Oracle FSDRNN (uses true reduction B_0)
    # ========================================================================
    if verbose:
        print("  • Oracle FSDRNN (with true reduction B_0)...")
    start_time = time.time()
    oracle_method = OracleFSdrnnWrapper(20, 8, B_true=beta, lr=5e-4, epochs=1000, dropout=0.1, device=device, verbose=False)
...
def main():
    parser = argparse.ArgumentParser(
        description='Linear-Nonlinear SDR Simulation: Compare 5 Methods'
    )
